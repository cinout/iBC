import torch
from torch import nn


# implementation reference:https://github.com/GeniusHTX/CLUB
class DV(nn.Module):
    def __init__(self, TNet):
        super(DV, self).__init__()
        self.F_func = TNet

    def forward(self, feature_maps, repres):

        # shuffle repres
        sample_size = repres.shape[0]
        repres_shuffle = repres[torch.randperm(sample_size)]

        T0 = self.F_func(
            feature_maps, repres
        )  # only 1 channel because of self.F_func (TNet)
        T1 = self.F_func(feature_maps, repres_shuffle)  # same

        lower_bound = T0.mean() - torch.log(T1.exp().mean())
        return lower_bound

    """
    Input:
        feature_maps: flattned internal hooks outputs
        repres: backbone_ouput + torch.flatten(x, start_dim=1) and then F.normalize(feature, dim=-1), shape: [bs, dim]
    """

    def learning_loss(self, feature_maps, repres):
        return -self.forward(feature_maps, repres)
